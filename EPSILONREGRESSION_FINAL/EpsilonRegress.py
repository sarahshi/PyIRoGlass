...
ax.plot(tau_arr_5200, epsilon_5200_arr, 'k', lw = 2, zorder = 0, label = f'Epsilon5200={round(mest_5200[0],3)}(+{round(np.sqrt(np.diag(covm_est_5200))[0],3)})+{round(mest_5200[1],3)}(+{round(np.sqrt(np.diag(covm_est_5200))[1],3)})Tau, N={len(tau_5200)}')
...
ax.fill_between(tau_arr_5200[fuego_idx], conf_lower_5200[fuego_idx], conf_upper_5200[fuego_idx], color = 'r', alpha=0.30,
...
ax.set_xlim([ 0.5, 1.0])
ax.set_ylim([-0.5, 3.5])
ax.set_xlabel('Tau=(Si+Al)/Total Cations') 
ax.set_ylabel('EpsilonH2Om,5200')
...
sz = 150
fig, ax = plt.subplots(1, 1, figsize = (8, 8))
ax.plot(tau_arr_4500, epsilon_4500_arr, 'k', lw = 2, zorder = 0, label = f'Epsilon4500={round(mest_4500[0],3)}(+{round(np.sqrt(np.diag(covm_est_4500))[0],3)})+{round(mest_4500[1],3)}(+{round(np.sqrt(np.diag(covm_est_4500))[1],3)})Tau, N={len(tau_4500)}')
...
sz = 150
fig, ax = plt.subplots(1, 1, figsize = (8, 8))
ax.plot(tau_arr_3550, epsilon_3550_arr, 'k', lw = 2, zorder = 0, label = f'Epsilon3550={round(mest_3550[0],3)}(+{round(np.sqrt(np.diag(covm_est_3550))[0],3)})+{round(mest_3550[1],3)}(+{round(np.sqrt(np.diag(covm_est_3550))[1],3)})Tau, N={len(tau_3550)}')
mand.set_dashes([1.5, 1, 3, 1])
...
ax.set_ylim([20, 110])
ax.set_xlabel('Tau=(Si+Al)/Total Cations') 
ax.set_ylabel('EpsilonH2Ot,3550')
ax.legend(loc = 'upper left', labelspacing = 0.4, handletextpad = 0.5, handlelength = 1.50, prop={'size': 12}, frameon=False)
ax.tick_params(axis="x", direction='in', length=5, pad = 6.5)
ax.tick_params(axis="y", direction='in', length=5, pad = 6.5)
plt.tight_layout()
...
sz = 150
fig, ax = plt.subplots(1, 1, figsize = (8, 8))
ax.plot(tau_arr_1635, epsilon_1635_arr, 'k', lw = 2, zorder = 0, label = f'Epsilon1635={round(mest_1635[0],3)}(+{round(np.sqrt(np.diag(covm_est_1635))[0],3)})+{round(mest_1635[1],3)}(+{round(np.sqrt(np.diag(covm_est_1635))[1],3)})Tau, N={len(tau_1635)}')
...
ax.set_ylim([0, 90])
ax.set_xlabel('Tau=(Si+Al)/Total Cations') 
ax.set_ylabel('EpsilonH2Om,1635')
ax.legend(loc = 'upper left', labelspacing = 0.4, handletextpad = 0.5, handlelength = 1.50, prop={'size': 12}, frameon=False)
ax.tick_params(axis="x", direction='in', length=5, pad = 6.5)
ax.tick_params(axis="y", direction='in', length=5, pad = 6.5)
plt.tight_layout()
...
ax.scatter(low_df['Na/Na+Ca'], low_df['Epsilon_Carbonate'], s = sz, c = '#0C7BDC', edgecolors='black', linewidth = 0.5, zorder = 15, label = 'CO32- 1430')
...
ax.scatter(high_df['Na/Na+Ca'], high_df['Epsilon_Carbonate'], s = sz, c = '#E42211', marker = 's', edgecolors='black', linewidth = 0.5, zorder = 15, label = 'CO32- 1515')
...
ax.plot(naca_arr, epsilon_carbonate_arr, 'k', lw = 2, zorder = 0, label = f'CO32-={round(mest_carbonate[0],3)}(+{round(np.sqrt(np.diag(covm_est_carbonate))[0],3)})-{round(mest_carbonate[1],3)*-1}(+{round(np.sqrt(np.diag(covm_est_carbonate))[1],3)})Na/(Na+Ca), N={len(naca)}')
...
ax.fill_between(naca_arr[fuego_idx], conf_lower_carbonate[fuego_idx], conf_upper_carbonate[fuego_idx], color = 'r', alpha=0.30,
    zorder = -5, label='Fuego Interval')
ax.set_xlim([0.1, 0.9])
ax.set_ylim([0, 500])
ax.set_xlabel('Na/(Na+Ca)') 
ax.set_ylabel('EpsilonCO32-')
